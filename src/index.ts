...
server.registerTool(
  ...
  async ({ type, amount, category, description, date }) => {
    const stmt = db.prepare(
      "INSERT INTO txn (type, amount, category, description, date) VALUES (?, ?, ?, ?, ?)"
    );
...
server.registerTool(
  ...
  async () => {
    const incomeStmt = db.prepare(
      "SELECT SUM(amount) as total FROM txn WHERE type = 'income'"
    );
    const expenseStmt = db.prepare(
      "SELECT SUM(amount) as total FROM txn WHERE type = 'expense'"
    );

    const income = incomeStmt.get() as { total: number | null };
    const expense = expenseStmt.get() as { total: number | null };
  ...
  async ({ id, ids, category, type, startDate, endDate, olderThan, confirmBulk }) => {
    ...
    if (ids && ids.length > 0) {
      if (!confirmBulk) {
        ...
        const preview = previewStmt.all(...ids) as any[];
    ...
    const matchingTransactions = previewStmt.all(...params) as any[];
        ...
        content: [
          {
            ...
            text: `⚠️ **Bulk Deletion Confirmation Required**\n\n` +
                  ...
                  matchingTransactions.slice(0, 5).map(t => 
                    `- #${t.id}: ${t.type} $${t.amount} (${t.category}) - ${new Date(t.date).toLocaleDateString()}`
                  ).join('\n') +
      ...
      content: [
        {
          ...
          text: `✅ Successfully deleted ${result.changes} transaction(s)\n\n` +
                ...
                matchingTransactions.slice(0, 10).map(t => 
...
server.registerTool(
  ...
  async ({ chartType = "bar", groupBy = "month" }) => {
    const rows = db.prepare("SELECT * FROM txn ORDER BY date ASC").all() as Array<{
      id: number;
      type: string;
      amount: number;
      category: string;
...
server.registerResource(
  ...
  async () => {
    const rows = db.prepare("SELECT * FROM txn ORDER BY date DESC").all() as Array<{
      id: number;
      type: string;
      amount: number;
      category: string;
